...
inline void ComputeArcLength(const Eigen::MatrixXd& xyz, Eigen::MatrixXd& arclength)
{
    // Compute the arc-length
    arclength.resize(xyz.rows(),1);
    arclength.setZero();
    for(int i=0;i<xyz.rows()-1;i++)
        arclength(i+1,0) = (xyz.row(i+1) - xyz.row(i)).norm() + arclength(i,0);
    // Normalize
    double tot_length = arclength(xyz.rows()-1,0);
    for(int i=0;i<arclength.size();i++)
        arclength(i,0) = arclength(i,0)/tot_length;
}
